...
class CtpMdApi(MdApi):
    ...
    def onFrontDisconnected(self, reason: int) -> None:
        ...
        self.gateway.write_log(f"行情服务器 {self.address} 连接断开，原因{reason}")

    def onRspUserLogin(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        """用户登录请求回报"""
        if not error["ErrorID"]:
            self.login_status = True
            self.gateway.write_log(f"行情服务器 {self.address} 登录成功")

            for symbol in self.subscribed:
                self.subscribeMarketData(symbol)
        else:
            self.gateway.write_error(f"行情服务器 {self.address} 登录失败", error)
...
class CtpTdApi(TdApi):
    ...
    def onFrontDisconnected(self, reason: int) -> None:
        """服务器连接断开回报"""
        self.login_status = False
        self.gateway.write_log(f"交易服务器 {self.address} 连接断开，原因{reason}")

    def onRspAuthenticate(self, data: dict, error: dict, reqid: int, last: bool) -> None:
        """用户授权验证回报"""
        if not error['ErrorID']:
            self.auth_status = True
            self.gateway.write_log(f"交易服务器 {self.address} 授权验证成功")
            self.login()
        else:
            # 如果是授权码错误，则禁止再次发起认证
            if error['ErrorID'] == 63:
            ...
            self.gateway.write_error(f"交易服务器 {self.address} 授权验证失败", error)
