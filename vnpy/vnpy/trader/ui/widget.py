...
class ConnectDialog(QtWidgets.QDialog):
    ...
    def __init__(self, main_engine: MainEngine, gateway_name: str) -> None:
        ...
        self.accounts_filename = f"last_connect_{gateway_name.lower()}.json"
        self.main_engine: MainEngine = main_engine
    ...
    def init_ui(self) -> None:
        ...
        self.all_accounts = load_json(self.filename)
        ...
        form.addRow(_("选择账号"), self.account_combo)
        self.account_combo.setMinimumWidth(200)
        self.account_combo.setSizeAdjustPolicy(QtWidgets.QComboBox.SizeAdjustPolicy.AdjustToContents)
        if self.all_accounts:
            self.account_combo.setCurrentIndex(1)
            loaded_setting = self.all_accounts[self.account_combo.currentText()]

        # Initialize widgets for each field
        if default_setting:
        ...
        connect_btn: QtWidgets.QPushButton = QtWidgets.QPushButton(_("连接"))
        connect_btn.clicked.connect(self.connect_gateway)
        button_layout.addWidget(connect_btn)

        delete_account_btn: QtWidgets.QPushButton = QtWidgets.QPushButton(_("删除账号"))
    ...
    def on_account_changed(self, account_name: str) -> None:
        """Load selected account settings."""
        if account_name == _("新建账号"):
            ...
            # Load selected account settings
            if account_name in self.all_accounts:
                account_setting = self.all_accounts[account_name]
                for field_name, value in account_setting.items():
                    if field_name in self.widgets:
                        widget, field_type = self.widgets[field_name]
                        if isinstance(widget, QtWidgets.QLineEdit):
                            widget.setText(str(value))
                        elif isinstance(widget, QtWidgets.QComboBox):
                            ix = widget.findText(str(value))
    ...
    def get_settings(self) -> dict:
        ...
        for field_name, (widget, field_type) in self.widgets.items():
            ...
            if field_type is int:
                settings[field_name] = int(value_str)
            else:
                settings[field_name] = value_str
    ...
    def delete_account(self) -> None:
        ...
        if current_account == _("新建账号"):
            QtWidgets.QMessageBox.warning(self, _("警告"), _("无法删除新建账号"))
            return

        reply = QtWidgets.QMessageBox.question(
            self,
            _("确认删除"),
            _("确定要删除账号 {} 吗?").format(current_account)
        )

        if reply == QtWidgets.QMessageBox.StandardButton.Yes:
            if current_account in self.all_accounts:
                del self.all_accounts[current_account]
                save_json(self.filename, self.all_accounts)

                index = self.account_combo.findText(current_account)
                self.account_combo.removeItem(index)
                self.account_combo.setCurrentIndex(0)
                QtWidgets.QMessageBox.information(self, _("成功"), _("账号删除成功"))

    def load_or_init_accounts(self) -> dict:
        """Load accounts from file or initialize empty list."""
        try:
            accounts = load_json(self.filename)
    ...
    def connect_gateway(self) -> None:
        ...
        accounts = self.load_or_init_accounts()
        ...
        for field_name, tp in self.widgets.items():
            ...
            # Check if account already exists, update or create
            if setting['用户名']:
                accounts[setting['用户名']] = setting

        save_json(self.filename, accounts)
...
class TradingWidget(QtWidgets.QWidget):
    ...
    def init_ui(self) -> None:
        ...
        send_button: QtWidgets.QPushButton = QtWidgets.QPushButton(_("加多"))
        send_button.clicked.connect(self.send_order)

        send_button_1: QtWidgets.QPushButton = QtWidgets.QPushButton(_("锁仓"))
        send_button_1.clicked.connect(self.send_order)

        send_button_2: QtWidgets.QPushButton = QtWidgets.QPushButton(_("平仓"))
        send_button_2.clicked.connect(self.send_order)

        cancel_button: QtWidgets.QPushButton = QtWidgets.QPushButton(_("撤单"))
        cancel_button.clicked.connect(self.cancel_all)

        cancel_button_1: QtWidgets.QPushButton = QtWidgets.QPushButton(_("暂停交易"))
        cancel_button_1.clicked.connect(self.cancel_all)
        ...
        grid.addWidget(send_button_1, 10, 0, 1, 3)
        grid.addWidget(send_button_2, 11, 0, 1, 3)
        grid.addWidget(cancel_button, 12, 0, 1, 3)
        grid.addWidget(cancel_button_1, 13, 0, 1, 3)
